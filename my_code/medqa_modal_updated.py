#!/usr/bin/env python3
"""
Modal-based MedQA evaluation with semantic uncertainty in the CoT setting.

This script runs a HuggingFace LLM on MedQA inside Modal with a GPU.

For each question, it:
  - Builds a CoT-style prompt ending with "Final answer:"
  - Gets a deterministic prediction (temperature=0)
  - Computes log probability of the predicted answer token
  - Samples M CoT answers at higher temperature
  - Extracts the rationale text for each sample
  - Embeds rationales and clusters them into semantic groups
  - Computes semantic entropy over clusters (semantic uncertainty)
  - Optionally also computes the empirical distribution over {A,B,C,D,E}
...
Columns in CSV:
  ...
  n_samples, semantic_entropy, n_clusters,
  p_A, p_B, p_C, p_D, p_E
...
Usage example (from your terminal):
  ...
  modal run medqa_modal.py \\
    ...
    --output_csv gemma_medqa_us_test_semantic.csv
"""
import os
import math
...
def evaluate_medqa_remote(
    ...
    Takes:
      ...
      - m_samples: number of samples for semantic entropy
    ...
    def build_base_prompt(example: Dict[str, Any]) -> str:
        """
        Build a multiple-choice CoT prompt for the MedQA example.

        We end with "Final answer:" so the next token(s) contain the answer letter.
        """
...
""".rstrip()
    ...
    def parse_rationale_and_letter(
        full_text: str,
        base_prompt: str,
    ) -> (str, Optional[str]):
        """
        Given the full decoded text and the base_prompt, return:
          - rationale: text after the prompt and before 'Final answer:'
          - letter: final answer letter (A/B/C/D/E), if present
        """
        # Remove the prompt prefix if it appears at the start
        if full_text.startswith(base_prompt):
            continuation = full_text[len(base_prompt):]
        else:
            continuation = full_text

        marker = "Final answer:"
        idx = continuation.rfind(marker)
        if idx == -1:
            # No explicit final answer marker found
            rationale = continuation.strip()
            letter = None
            return rationale, letter

        rationale = continuation[:idx].strip()
        after = continuation[idx + len(marker):].strip()
    ...
    def get_rationale_embeddings(
        ...
        """
        Compute sentence-level embeddings for each rationale via mean pooling.
        Returns [N, D] tensor (or None if no valid texts).
        """
        clean_texts = [t for t in texts if t is not None and t.strip() != ""]
        ...
        # Note: some entries in 'texts' may be empty; we align by skipping them upstream
        return embeddings

    def compute_semantic_entropy_from_embeddings(
        embeddings: torch.Tensor,
        sim_threshold: float = 0.8,
    ) -> (float, int):
        """
        Online clustering by cosine similarity with a given threshold.
        Returns (semantic_entropy, n_clusters).
        """
        import torch.nn.functional as F
        ...
        if N == 0:
            return float("nan"), 0
        if N == 1:
            # Only one rationale -> one cluster, zero entropy
            return 0.0, 1

        # Normalize for cosine similarity
        emb_norm = torch.nn.functional.normalize(embeddings, p=2, dim=1)

        cluster_centroids = []  # list of [D] tensors
        cluster_counts = []     # list of ints

        for i in range(N):
            e = emb_norm[i].unsqueeze(0)  # [1, D]

            if len(cluster_centroids) == 0:
                cluster_centroids.append(e.squeeze(0))
                cluster_counts.append(1)
                continue

            # Stack centroids and compute cosine similarity
            centroids_tensor = torch.stack(cluster_centroids, dim=0)  # [C, D]
            sims = F.cosine_similarity(e, centroids_tensor, dim=1)   # [C]

            max_sim, best_idx = torch.max(sims, dim=0)
            if float(max_sim.item()) >= sim_threshold:
                # Assign to existing cluster and update centroid (running mean)
                c = cluster_counts[best_idx]
                new_centroid = (cluster_centroids[best_idx] * c + e.squeeze(0)) / (c + 1)
                cluster_centroids[best_idx] = new_centroid
                cluster_counts[best_idx] = c + 1
            else:
                # New cluster
                cluster_centroids.append(e.squeeze(0))
                cluster_counts.append(1)

        C = len(cluster_counts)
        total = float(sum(cluster_counts))
        if total <= 0:
            return float("nan"), 0
        ...
        return float(entropy), C

    # ---------- Load models ---------- #

    print(f"[Modal] Loading main model: {model_name}")
    hf_token = os.environ.get("HF_TOKEN")

    tokenizer = AutoTokenizer.from_pretrained(model_name, token=hf_token)
    model = AutoModelForCausalLM.from_pretrained(model_name, token=hf_token)

    # Sentence embedding model for semantic uncertainty
    emb_model_name = "sentence-transformers/all-MiniLM-L6-v2"
    print(f"[Modal] Loading embedding model: {emb_model_name}")
    emb_tokenizer = AutoTokenizer.from_pretrained(emb_model_name, token=hf_token)
    emb_model = AutoModel.from_pretrained(emb_model_name, token=hf_token)

    if tokenizer.pad_token_id is None and tokenizer.eos_token_id is not None:
        tokenizer.pad_token = tokenizer.eos_token
    if emb_tokenizer.pad_token_id is None and emb_tokenizer.eos_token_id is not None:
    ...
    for i, ex in enumerate(examples):
        ...
        # 1) Deterministic answer (CoT style, but deterministic decoding)
        ...
        # 2) Logprob of predicted letter (next-token after prompt)
        ...
        # 3) Sampling for semantic entropy (CoT)
        ...
        # Parse rationales + letters for each sampled CoT
        rationales: List[str] = []
        sample_letters: List[Optional[str]] = []
        for t in sampled_texts:
            rationale, letter = parse_rationale_and_letter(t, base_prompt)
            if rationale is not None and rationale.strip() != "":
                rationales.append(rationale)
                sample_letters.append(letter)
            else:
                # still append letter for counting, but skip empty rationale later
                sample_letters.append(letter)

        # Compute semantic embeddings + entropy
        if len(rationales) > 0:
            emb = get_rationale_embeddings(rationales, emb_tokenizer, emb_model, device)
            if emb is not None:
                semantic_entropy, n_clusters = compute_semantic_entropy_from_embeddings(
                    emb, sim_threshold=0.8
                )
            else:
                semantic_entropy, n_clusters = float("nan"), 0
        else:
            semantic_entropy, n_clusters = float("nan"), 0

        # Optionally: still compute empirical distribution over answer letters
        ...
        print(
            ...
            f"logprob={logprob_det}, semH={semantic_entropy:.3f}, "
...
def main(
    ...
    output_csv: str = "medqa_results_modal_semantic.csv",
):
    """
